...
import json
from app_description_scraper import scrape_app_data
...
input_file_path = 'package_names.csv'  # Change this to your file path

df = pd.read_csv(input_file_path).head(50)
package_names = df['Package_name'].tolist()
app_data = [scrape_app_data(pkg) for pkg in package_names if scrape_app_data(pkg) is not None]

scraped_app_data = pd.DataFrame(app_data)
scraped_app_data.to_csv("scraped_app_data.csv", index=False)
